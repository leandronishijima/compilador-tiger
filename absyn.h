...
struct A_exp_
      {enum {A_varExp, A_nilExp, A_intExp, A_stringExp, A_callExp,
       ...
       union {A_var var;
	      ...
	      struct {S_symbol var; A_exp lo,hi,body; int escape;} forr;
...
struct A_dec_ 
    {enum {A_functionDec, A_varDec, A_typeDec} kind;
     ...
     union {A_fundecList function;
	    ...
	    struct {S_symbol var; S_symbol typ; A_exp init; int escape;} var;
	    A_nametyList type;
	  } u;
   };
...
struct A_field_ {S_symbol name, typ; A_pos pos; int escape;};
...
A_dec A_FunctionDec(A_pos pos, A_fundecList fundecList);
